...
class DeepGlobalRegistration:
    ...
    def preprocess(self, pcd):
        ...
        coords = ME.utils.batched_coordinates(
            [torch.floor(xyz / self.voxel_size).int()])
        feats = torch.ones(npts, 1)

        return xyz.float(), coords, feats

    def fcgf_feature_extraction(self, feats: torch.Tensor, coords: torch.Tensor):
        '''
        Step 1: extract fast and accurate FCGF feature per point
        '''
        sinput = ME.SparseTensor(features=feats.to(self.device), coordinates=coords.to(self.device))
    ...
    def inlier_feature_generation(self, xyz0, xyz1, coords0, coords1, fcgf_feats0,
        ...
        if feat_type == 'ones':
            ...
            feat = torch.cat(
                (fcgf_feats0[corres_idx0], fcgf_feats1[corres_idx1]), dim=1)
    ...
    def inlier_prediction(self, inlier_feats: torch.Tensor, coords: torch.Tensor):
        '''
        Step 4: predict inlier likelihood
        '''
        sinput = ME.SparseTensor(features=inlier_feats.to(self.device), coordinates=coords.to(self.device))
    ...
    def register(self, xyz0, xyz1, inlier_thr=0.00):
        ...
        with torch.no_grad():
            ...
            corres_idx0, corres_idx1 = self.fcgf_feature_matching(
            ...
            logit = self.inlier_prediction(
                inlier_feats.contiguous(), coords=inlier_coords)
