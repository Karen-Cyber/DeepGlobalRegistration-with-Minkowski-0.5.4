...
class WeightedProcrustesTrainer:
    ...
    def _train_epoch(self, epoch):
        ...
        for curr_iter in range(num_train_iter):
            ...
            for iter_idx in range(iter_size):
                ...
                loss = self.config.procrustes_loss_weight * \
                    individual_loss[valid_mask].mean()
                ...
                with torch.no_grad():
                    ...
                    max_grad = torch.abs(
                        self.inlier_model.final.kernel.grad).max().cpu().item()
            ...
            if curr_iter % self.config.stat_freq == 0:
                ...
                for k, v in stat.items():
                    self.writer.add_scalar(
                        f'train/{k}', v, start_iter + curr_iter)
    ...
    def _load_weights(self, config):
        if config.resume is None and config.weights:
            logging.info(
                "=> loading weights for inlier model '{}'".format(config.weights))
            checkpoint = torch.load(config.weights)
            self.feat_model.load_state_dict(checkpoint['state_dict'])
            logging.info(
                "=> Loaded base model weights from '{}'".format(config.weights))
            if 'state_dict_inlier' in checkpoint:
                self.inlier_model.load_state_dict(
                    checkpoint['state_dict_inlier'])
                logging.info(
                    "=> Loaded inlier weights from '{}'".format(config.weights))
            else:
                logging.warn(
                    "Inlier weight not found in '{}'".format(config.weights))
        ...
        if config.resume is not None:
            if osp.isfile(config.resume):
                ...
                if 'state_dict_inlier' in state:
                    self.inlier_model.load_state_dict(
                        state['state_dict_inlier'])
                    self.inlier_model = self.inlier_model.to(self.device)
                else:
                    logging.warn(
                        "Inlier weights not found in '{}'".format(config.resume))
            else:
                logging.warn(
                    "Inlier weights does not exist at '{}'".format(config.resume))
    ...
    def generate_inlier_input(self, xyz0, xyz1, iC0, iC1, iF0, iF1, len_batch, pos_pairs):
        ...
        is_correct = find_correct_correspondence(
            pos_pairs, pred_pairs, len_batch=len_batch)
