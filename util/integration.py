...
def process_seq(seq_path, output_path, n_frames_per_fragment, display=False):
    ...
    if 'intrinsics.txt' in files:
        fx, fy, cx, cy = read_intrinsics(
            os.path.join(seq_path, 'intrinsics.txt'))
    else:
        fx, fy, cx, cy = read_intrinsics(
            os.path.join(seq_path, '../camera-intrinsics.txt'))

    rgb_files = sorted(list(filter(lambda x: x.endswith('.color.png'), files)))
    depth_files = sorted(
        list(filter(lambda x: x.endswith('.depth.png'), files)))
    ...
    n_fragments = ((len(rgb_files) + n_frames_per_fragment - 1)
                   ) // n_frames_per_fragment
    ...
    for fragment_id in range(0, n_fragments):
        ...
        mesh_name = os.path.join(
            output_seq_path, 'fragment-{}.ply'.format(fragment_id))
...
if __name__ == '__main__':
    ...
    seqs = list(filter(lambda x: x.startswith(
        'seq'), os.listdir(args.dataset)))
    for seq in seqs:
