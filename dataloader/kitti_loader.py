...
class KITTIPairDataset(PairDataset):
    ...
    def pos_transform(self, pos):
        ...
        RT = np.eye(4)
        RT[:3, :3] = np.dot(
            np.dot(self.rot3d(0, rx), self.rot3d(1, ry)), self.rot3d(2, rz))
        RT[:3, 3] = [x, y, z]
        return RT
    ...
    def __getitem__(self, idx):
        ...
        positions = [self.odometry_to_positions(
            odometry) for odometry in all_odometry]
        ...
        sel0 = ME.utils.sparse_quantize(
            xyz0_th / self.voxel_size, return_index=True)
        sel1 = ME.utils.sparse_quantize(
            xyz1_th / self.voxel_size, return_index=True)
...
class KITTINMPairDataset(KITTIPairDataset):
    ...
    def __init__(self,
        ...
        for dirname in subset_names:
            ...
            all_pos = np.array([self.odometry_to_positions(odo)
                               for odo in all_odo])
            Ts = all_pos[:, :3, 3]
