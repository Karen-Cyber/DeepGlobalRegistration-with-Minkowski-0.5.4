...
def plot_precision_recall_curves(stats, method_names, rte_precisions, rre_precisions,
    ...
    for i, rre_thresh in enumerate(rre_precisions):
        pairwise_stats = analyze_by_pair(
            stats, rte_thresh=np.inf, rre_thresh=rre_thresh)
        rre_precision_curves[:, i] = pairwise_stats[:, 0]

    for i, rte_thresh in enumerate(rte_precisions):
        pairwise_stats = analyze_by_pair(
            stats, rte_thresh=rte_thresh, rre_thresh=np.inf)
    ...
    for m, name in enumerate(method_names):
        ...
        ax1.plot(rre_precisions,
                 rre_precision_curves[m], color=cmap[m], alpha=alpha)
        ax2.plot(rte_precisions,
                 rte_precision_curves[m], color=cmap[m], alpha=alpha)
